...
| Маршрутизация запросов | Подсистема балансировки нагрузки | Распределение нагрузки между зонами | Диспетчер трафика |
| Задержки сети | Очень низкий | Низкий | От среднего до высокого |
| Виртуальная сеть  | Виртуальная сеть | Виртуальная сеть | Пиринговая связь между виртуальными сетями между регионами (предварительная версия) |
...
### <a name="availability-zones-preview"></a>Зоны доступности (предварительная версия)
...
В Azure нет эквивалентов для точечных экземпляров EC2 или выделенных узлов.
