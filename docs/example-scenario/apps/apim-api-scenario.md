...
* Внутренняя команда разработчиков создаст новую бизнес-функцию, которая будет предоставляться по новым API протокола HTTP.
...
Новые возможности приложения будут доставлены поэтапно. Они будут *постепенно заменять* существующие функции пользовательского интерфейса клиент-сервер на основе браузера (размещены локально), которые бизнес-компания электронной коммерции использует сегодня.
...
Чтобы просмотреть прогнозируемые затраты и настроить потребности развертывания, вы можете изменить количество единиц масштабирования и экземпляров службы приложений в [Калькуляторе цен Azue][pricing-calculator].

## <a name="related-resources"></a>Связанные ресурсы

Ознакомьтесь с обширной документацией по управлению API Azure в статье[Что такое управление API.][apim]
