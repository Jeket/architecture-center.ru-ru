...
* [Виртуальные машины Linux Azure][azurevm-docs] — это платформа IaaS для запуска экземпляров Jenkins и Grafana.
* [Реестр контейнеров Azure][azureacr-docs] хранит данные и управляет образами контейнеров, которые использует кластер службы контейнеров Azure. Изображения надежно сохранены и могут быть реплицированы платформой Azure в другие регионы для ускорения развертывания.
