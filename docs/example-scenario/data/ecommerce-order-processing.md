...
description: Пример сценария с использованием Azure Cosmos DB для создания конвейера обработки заказа с высокой степенью масштабируемости.
author: alexbuckgit
ms.date: 07/10/2018
...
В этом сценарии применяется подход источника событий с использованием модели функционального программирования, реализованной с помощью микрослужб. Каждая микрослужба считается обработчиком потоков и вся бизнес-логика реализуется через микрослужбы. Этот подход обеспечивает высокий уровень доступности и устойчивости, георепликацию и высокую производительность.
...
![Пример архитектуры для конвейера масштабируемой обработки заказов][architecture-diagram]
...
[Модель безопасности Cosmos DB](/azure/cosmos-db/secure-access-to-data) проверяет подлинность пользователей и предоставляет доступ к своим данным и ресурсам. Дополнительные сведения см. в статье [Безопасность базы данных в Azure Cosmos DB](/en-us/azure/cosmos-db/database-security).
...
* [Небольшой][small-pricing]. Коррелируется с 5 ЕЗ, зарезервированными для использования с хранилищем данных объемом 1 ТБ в Cosmos DB и небольшим (D3 v2) кластером Kafka.
* [Средний][medium-pricing]. Коррелируется с 50 ЕЗ, зарезервированными для использования с хранилищем данных объемом 10 ТБ в Cosmos DB и кластером Kafka (D4 v2) среднего размера.
* [Большой][large-pricing]. Коррелируется с 500 ЕЗ, зарезервированными для использования с хранилищем данных объемом 30 ТБ в Cosmos DB и кластером Kafka (D5 v2) большого размера.
...
* _[Designing Data-Intensive Applications](https://dataintensive.net/)_ (Разработка ресурсоемких приложений), Мартин Клеппманн (Martin Kleppmann) (O'Reilly Media, 2017).
* _[Domain Modeling Made Functional: Tackle Software Complexity with Domain-Driven Design and F#](https://pragprog.com/book/swdddf/domain-modeling-made-functional)_ (Функциональное моделирование доменов: упрощение программного обеспечения с поддержкой домена и F#), Скотт Влашин (Scott Wlaschin) (Pragmatic Programmers LLC, 2018).
