...
Этот подход используется для данных, которые часто изменяются. Ниже приведен предыдущий пример, однако теперь в нем используется шаблон "кэш на стороне".  
...
- [Шаблон "Кэш на стороне"][cache-aside-pattern]
