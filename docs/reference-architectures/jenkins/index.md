...
*Скачайте [файл Visio](https://arch-center.azureedge.net/cdn/Jenkins-architecture.vsdx), который содержит схему этой архитектуры.*
...
-   **Группы безопасности сети.** С помощью [групп безопасности сети][nsg] (NSG) можно ограничить трафик, направляющийся из Интернета в подсеть виртуальной сети.
...
-   Подключаемый модуль [агентов виртуальных машин Azure][configure-agent] создает агенты Jenkins на виртуальных машинах Azure с помощью шаблона Azure Resource Manager (ARM).
...
Оцените требования к доступности для своего рабочего процесса и продумайте, как восстановить состояние Jenkins, если главный сервер выйдет из строя. Для оценки требований к доступности рекомендуем использовать две общие метрики:
...
На практике под терминами RTO и RPO подразумеваются избыточность и резервное копирование. Доступность означает не восстановление оборудования — оно относится к платформе Azure, — а поддержание состояния сервера Jenkins. В этой эталонной архитектуре применяется [Соглашение об уровне обслуживания Azure][sla] (SLA), которое гарантирует 99,9 % времени непрерывной работы для отдельно взятой виртуальной машины. Если это соглашение не соответствует вашим требованиям ко времени непрерывной работы, разработайте план аварийного восстановления или выполните развертывание [с несколькими главными серверами Jenkins][multi-master] (не рассматривается в этом документе).
...
-   Настройте безопасный вход на сервер Jenkins. Протокол HTTP не является безопасным по умолчанию. В этой архитектуре используется HTTP и общедоступный IP-адрес. Рекомендуем настроить [HTTPS на сервере Nginx][nginx] для безопасного входа.
...
-   Создайте профиль безопасности, который определяет ресурсы, необходимые пользователям, службам и агентам конвейеров для выполнения заданий (но не более того). Этот шаг становится критически важным при настройке параметров безопасности.
...
Azure предоставляет несколько функций для [мониторинга и диагностики][monitoring-diag] во всей инфраструктуре. Для наблюдения за загрузкой ЦП в этой архитектуре развернута служба Azure Monitor. С ее помощью, например, можно отслеживать загрузку ЦП и настроить отправку уведомлений в случае, если этот показатель превысит 80 %. (Высокая загрузка ЦП указывает на то, что может потребоваться масштабирование виртуальной машины с сервером Jenkins.) Также можно уведомить указанного пользователя в случае неисправности или недоступности виртуальной машины.
...
1.  Откройте [образ Azure Marketplace для Jenkins][azure-market] в браузере и нажмите кнопку **ПОЛУЧИТЬ** в левой части страницы.
...
Используйте [подключаемый модуль Azure AD][configure-azure-ad] из центра обновления Jenkins на сервере Jenkins и следуйте инструкциям по настройке единого входа.
