...
В этом примере разработчик работает над микрослужбой, которая называется "Служба доставки". (Имя из эталонной реализации, описанной [здесь](../../microservices/design/index.md#scenario).) При разработке новых компонентов разработчик проверяет код в ветви компонентов.
...
1. Повторно маркирует образ Docker для пространства имен рабочей среды в Реестре контейнеров Azure. Например, если текущий маркер – `myrepo.azurecr.io/delivery:v1.0.2`, рабочим маркером будет `myrepo.azurecr.io/prod/delivery:v1.0.2`.
