...
На следующей схеме показан возможный рабочий процесс CI/CD. В этом примере предполагается, что существует роль контроля качества, которая отделена от роли разработчика.

![Рабочий процесс CI/CD](./_images/aks-cicd.png)

1. Разработчик совершает изменение.
1. Это изменение запускает конвейер CI. Этот конвейер создает код, запускает тесты и создает образ контейнера.
1. Если все шлюзы пройдены, образ перемещается в репозиторий.
1. Когда новая версия службы готова к развертыванию, добавляется тег.
1. Этот тег запускает тестовый конвейер непрерывного развертывания, который запускает команду helm для обновления тестового кластера.
1. Если новая версия готова к развертыванию в рабочей среде, пользователь с ролью контроля качества вручную запускает конвейер непрерывного развертывания в рабочей среде.

### <a name="recommended-cicd-practices"></a>Рекомендации по CI/CD

Используйте для параметра imagePullPolicy значение Always, чтобы Kubernetes всегда извлекал последний образ из репозитория и не использовал кэшированный. Это можно применить в кластере с помощью контроллера допуска AlwaysPullImages.

Не используйте тег `latest` для образов в спецификации модуля. Всегда указывайте версию образа.

Используйте пространство имен в Службе контейнеров Azure для упорядочения образов контейнеров по микрослужбе или группе разработчиков.
