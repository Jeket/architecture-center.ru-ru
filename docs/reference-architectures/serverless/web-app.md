...
1. Ценообразование на основе потребления: вы платите только за вычислительные ресурсы, которые используются для выполнения кода.
...
- **CDN.** По умолчанию Azure CDN поддерживает HTTPS в поддомене `*.azureedge.net`. Чтобы включить HTTPS в сети CDN для имен личных доменов, ознакомьтесь со статьей [Руководство по настройке протокола HTTPS для личного домена в сети доставки содержимого Azure][cdn-https]. 
