...
На схеме эталонной архитектуры представлен набор методик, используемых для запуска нескольких виртуальных машин Linux в масштабируемом наборе в подсистеме балансировки нагрузки и повышающих доступность и масштабируемость. Эта архитектура может использоваться для любой рабочей нагрузки без учета состояния, такой как веб-сервер, и является строительным блоком при развертывании n-уровневых приложений. [**Разверните это решение**.](#deploy-the-solution)
...
Эта архитектура создана на основе архитектуры, описанной в статье [Запуск виртуальной машины Linux в Azure][single vm]. Рекомендации также применимы к этой архитектуре.
...
При наличии нескольких виртуальных машин важно автоматизировать процессы, чтобы они были надежными и повторяемыми. Для автоматизации развертывания можно использовать [службу автоматизации Azure][azure-automation], исправления ОС и другие задачи.
...
Пример развертывания для этой архитектуры можно найти на портале [GitHub][github-folder]. Он включает виртуальную сеть, группу безопасности сети и три виртуальные сети в масштабируемом наборе за подсистемой балансировки нагрузки, описанной ниже:

  * виртуальная сеть с одной подсетью, называющейся **web**, используемая для размещения виртуальных машин;
  * масштабируемый набор виртуальной машины, который содержит виртуальные машины, выполняющиеся под управлением последней версии Ubuntu 16.04.3 LTS; автомасштабирование включено;
  * подсистема балансировки нагрузки, которая находится перед масштабируемым набором виртуальной машины;
  * группа безопасности сети с правилами для входящего трафика, позволяющими передавать HTTP-трафик в масштабируемый набор виртуальных машин.
...
1. Перейдите в папку `virtual-machines\multi-vm\parameters\linux` репозитория, скачанного на предыдущем шаге.
