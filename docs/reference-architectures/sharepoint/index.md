...
- 8 серверов SharePoint на ВМ размера Standard_DS3_v2 (4 ядра для каждого) = 32 ядра
- 2 контроллера домена Active Directory на ВМ размера Standard_DS1_v2 (1 ядро для каждого) = 2 ядра
- 2 виртуальные машины размера Standard_DS3_v2 с SQL Server = 8 ядер
- 1 узел большинства на ВМ размера Standard_DS1_v2 = 1 ядро
- 1 сервер управления на ВМ размера Standard_DS1_v2 = 1 ядро
...
На этом этапе проверьте, можно ли установить подключение TCP между веб-интерфейсом и подсистемой балансировки нагрузки для группы доступности AlwaysOn на платформе SQL Server. Для этого выполните следующие действия:
