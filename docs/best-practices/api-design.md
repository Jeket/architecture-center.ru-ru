...
Серверу дается указание обновить `price`, удалить `color` и добавить `size` &mdash; `name` и `category` не изменяются. Точные сведения об исправлении со слиянием JSON см. в документе [RFC 7396](https://tools.ietf.org/html/rfc7396). Тип носителя для исправления со слиянием JSON — `application/merge-patch+json`.
...
> [!NOTE]
> Некоторые браузеры прежних версий и веб-прокси не могут кэшировать ответы по запросам, содержащим строку запроса в URL-адресе. Это может привести к снижению производительности для веб-приложений, которые используют веб-API и выполняемых через такой веб-браузер.
