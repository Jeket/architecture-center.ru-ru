...
* Отдел — имя для части организации, в которой работает группа сотрудников. Данный элемент необязателен в пространстве имен.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Группа ресурсов |Группа ресурсов |Глобальные |От 1 до 64 |Без учета регистра |Буквы, цифры, символ подчеркивания, круглые скобки, дефис и точка (но не в конце имени) |`<service short name>-<environment>-rg` |`profx-prod-rg` |
...
| Среда выполнения приложений |Виртуальная машина |Группа ресурсов |1–15 (Windows), 1–64 (Linux) |Без учета регистра |Буквы, цифры, символ подчеркивания и дефис |`<name>-<role>-vm<number>` |`profx-sql-vm1` |
| Среда выполнения приложений |Приложение-функция | Глобальные |1–60 |Без учета регистра |Буквы, цифры, дефис |`<name>-func` |`calcprofit-func` |
| Хранилище |Имя учетной записи хранения (данные о ней) |Глобальные |От 3 до 24 |Нижний регистр |Буквенно-цифровой |`<globally unique name><number>` (используйте функцию для расчета уникального идентификатора для именования учетных записей хранения) |`profxdata001` |
...
| Хранилище |Имя таблицы | Учетная запись хранения |От 3 до 63 |Без учета регистра |Буквенно-цифровой |`<service short name><context>` |`awesomeservicelogs` |
...
| Хранилище |Data Lake Store | Глобальные |От 3 до 24 |Нижний регистр | Буквенно-цифровой |`<name>-dtl` |`telemetry-dtl` |
| Сеть |Виртуальная сеть (VNet) |Группа ресурсов |2–64 |Не учитывает регистр. |Буквы, цифры, тире, символ подчеркивания, точка |`<service short name>-vnet` |`profx-vnet` |
| Сеть |Подсеть |Родительская виртуальная сеть |От 2 до 80 |Без учета регистра |Буквы, цифры, символ подчеркивания, тире и точка |`<descriptive context>` |`web` |
| Сеть |Сетевой интерфейс |Группа ресурсов |От 1 до 80 |Не учитывает регистр. |Буквы, цифры, тире, символ подчеркивания, точка |`<vmname>-nic<num>` |`profx-sql1-nic1` |
| Сеть |Группа безопасности сети |Группа ресурсов |От 1 до 80 |Не учитывает регистр. |Буквы, цифры, тире, символ подчеркивания, точка |`<service short name>-<context>-nsg` |`profx-app-nsg` |
| Сеть |Правило группы безопасности сети |Группа ресурсов |От 1 до 80 |Не учитывает регистр. |Буквы, цифры, тире, символ подчеркивания, точка |`<descriptive context>` |`sql-allow` |
| Сеть |Общедоступный IP-адрес |Группа ресурсов |От 1 до 80 |Не учитывает регистр. |Буквы, цифры, тире, символ подчеркивания, точка |`<vm or service name>-pip` |`profx-sql1-pip` |
| Сеть |Подсистема балансировки нагрузки |Группа ресурсов |От 1 до 80 |Не учитывает регистр. |Буквы, цифры, тире, символ подчеркивания, точка |`<service or role>-lb` |`profx-lb` |
| Сеть |Конфигурация правил балансировки нагрузки |Подсистема балансировки нагрузки |От 1 до 80 |Не учитывает регистр. |Буквы, цифры, тире, символ подчеркивания, точка |`<descriptive context>` |`http` |
| Сеть |Шлюз приложений Azure |Группа ресурсов |От 1 до 80 |Не учитывает регистр. |Буквы, цифры, тире, символ подчеркивания, точка |`<service or role>-agw` |`profx-agw` |
| Сеть |Профиль диспетчера трафика |Группа ресурсов |1–63 |Не учитывает регистр. |Буквы, цифры, тире, точка |`<descriptive context>` |`app1` |
...
Корпорация Майкрософт часто добавляет на платформу Azure новые службы. В таблице выше перечислены самые распространенные службы сети, вычислений и хранилища. Для других служб постарайтесь выбрать понятный суффикс из трех букв.

## <a name="organizing-resources-with-tags"></a>Упорядочение ресурсов с помощью тегов

Azure Resource Manager позволяет определять для сущностей теги с любыми текстовыми строками, которые упрощают определение контекста и автоматизацию.  Например, тег `"sqlVersion: "sql2014ee"` позволяет выделить в развертывании все виртуальные машины под управлением SQL Server 2014 Enterprise Edition, чтобы выполнять для них специальный скрипт автоматизации.  Теги должны использоваться для дополнения и улучшения контекста наряду с выбранным соглашением об именовании.
...
Вы можете настроить пользовательское доменное имя для доступа к данным больших двоичных объектов из учетной записи хранения Azure. Адрес конечной точки по умолчанию для службы BLOB-объектов — https://<name>.blob.core.windows.net`.
