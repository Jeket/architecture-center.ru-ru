...
---
# <a name="api-implementation"></a>Руководство по реализации API
...
## <a name="considerations-for-processing-requests"></a>Рекомендации относительно обработки запросов
...
* Запрос HTTP GET к URI `http://adventure-works.com/customers/2/orders` для поиска всех заказов заказчика. данные могут быть возвращены в формате XML или JSON;
* Запрос HTTP PUT к URI `http://adventure-works.com/customers/2/orders` для создания нового заказа для заказчика. данные должны быть представлены в сообщении запроса в формате x-www-form-urlencoded.
...
Клиентское приложение может выдать запрос на получение 30 заказов, начиная с позиции смещения 50, используя такой URI: `http://www.adventure-works.com/api/orders?limit=30&offset=50`.
...
## <a name="publish-and-manage-a-web-api-using-the-azure-api-management-service"></a>Публикация и управление веб-API с помощью службы управления API Azure
...
## <a name="support-developers-building-client-applications"></a>Поддержка разработчиков, создающих клиентские приложения
...
* В статье, посвященной [протоколу OData веб-API ASP.NET](http://www.asp.net/web-api/overview/odata-support-in-aspnet-web-api), есть примеры и дополнительные сведения о реализации веб-API OData с помощью ASP.NET.
* В статье [о внедрении поддержки пакетной службы в веб-API и веб-API OData](http://blogs.msdn.com/b/webdev/archive/2013/11/01/introducing-batch-support-in-web-api-and-web-api-odata.aspx) описывается, как реализовать поддержку пакетных операций в веб-API с помощью OData.
* В записи блога Джонатана Оливера (Jonathan Oliver) [о шаблонах идемпотентности](http://blog.jonathanoliver.com/idempotency-patterns/) описана концепция идемпотентности и ее связь с операциями управления данными.
* На веб-сайте консорциума W3C представлен полный список [кодов состояния HTTP](http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html) с определениями и описаниями.
