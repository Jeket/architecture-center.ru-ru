...
Схема секционирования может существенно повлиять на производительность системы. Она также может влиять на скорость добавления или удаления сегментов, а также повторного секционирования данных между сегментами. Вот несколько рекомендаций.
...
- Группируйте используемые вместе данные в одном сегменте и избегайте операций с доступом к данным, хранящимся в нескольких сегментах. Сегмент сам является базой данных SQL, а межбазовые соединения должны выполняться на стороне клиента.
    ...
    Несмотря на то, что база данных SQL не поддерживает межбазовые соединения, можно использовать средства эластичных баз данных для выполнения [запросы к нескольким сегментам](/azure/sql-database/sql-database-elastic-scale-multishard-querying). При запросе к нескольким сегментам отдельные запросы направляются к каждой базе данных, а полученные результаты объединяются.
...
Дополнительные сведения см. в статье [Руководство по проектированию хранилищ таблиц].
...
> [!IMPORTANT]
> Кэш Redis для Azure сейчас поддерживает кластеризацию Redis только на уровне [Премиум](/azure/azure-cache-for-redis/cache-how-to-premium-clustering).
...
[Руководство по проектированию хранилищ таблиц]: /azure/storage/storage-table-design-guide
