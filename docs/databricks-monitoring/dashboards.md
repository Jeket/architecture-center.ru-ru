...
---
# <a name="use-dashboards-to-visualize-azure-databricks-metrics"></a>Визуализация метрик Azure Databricks с помощью панелей мониторинга
...
Этот шаблон создает рабочую область, а также создает набор предопределенных запросов, которые используются панелью мониторинга.
...
1. Откройте веб-браузер и перейдите к следующему URL-АДРЕСУ: `http://<IP address>:3000`.
