---
title: Шаблон компенсирующих транзакций
...
> Этот подход аналогичен стратегии Саг, которая рассматривается в [блоге Клеменса Вастерса (Clemens Vasters)](https://vasters.com/clemensv/2012/09/01/Sagas.aspx).
...
- [Шаблон "планировщик, агент, контролер"](./scheduler-agent-supervisor.md). Описывает, как внедрять отказоустойчивые системы, которые выполняют бизнес-операции с использованием распределенных служб и ресурсов. Иногда может потребоваться отменить работу, выполняемую операцией, с помощью компенсирующей транзакции.
