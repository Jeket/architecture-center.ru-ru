...
> [!NOTE]
> Эта статья основана на вызове эталонную реализацию микрослужб [приложения доставки с помощью Дронов](./design/index.md).
...
Мы рассмотрели три варианта чтения и обработки сообщений: с помощью узла обработчика событий, очередей служебной шины и библиотеки IoTHub React. Мы выбрали IoTHub React, потому что она помогает начать работу с узлом обработчика событий.
...
## <a name="idempotent-versus-non-idempotent-operations"></a>Идемпотентные и неидемпотентные операции
...
## <a name="example-idempotent-operations"></a>Пример: Идемпотентные операции
