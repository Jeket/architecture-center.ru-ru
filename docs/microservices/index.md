...
Архитектура микрослужб широко используется для создания устойчивых, высокомасштабируемых и быстро развивающихся облачных приложений. Но это не просто специальный термин. Применение микрослужб требует другого подхода к разработке и созданию приложений. 
...
В рамках этого руководства мы рассмотрим полный сценарий: службу для доставки беспилотными летательными аппаратами, которая позволяет клиентам запланировать отгрузку и доставку посылок с помощью этих аппаратов. Код для реализации примера можно найти на портале GitHub.
...
- Микрослужба достаточно мала. Поэтому создать и обслуживать ее может небольшая команда разработчиков (как правило, 5&ndash;10 человек).
