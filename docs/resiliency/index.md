...
Вы должны определить свои собственные целевые соглашения SLA для каждой рабочей нагрузки в своем решении. SLA позволяет оценить, соответствует ли архитектура бизнес-требованиям. Выполните задачу по сопоставлению зависимостей, чтобы определить внутренние и внешние зависимости, например с Active Directory или сторонними службами, такими как служба платежей или электронной почты. В частности, обратите внимание на любые внешние зависимости, которые могут стать единой точкой отказа или узким местом во время инцидента. Например, если для рабочей нагрузки требуется 99,99 % времени безотказной работы, но она зависит от службы с SLA на уровне 99,9 %, эта служба не может быть единой точкой отказа в системе. Одним из средств защиты является наличие резервного пути в случае сбоя службы или принятие других мер для восстановления после сбоя в этой службе.
...
Например, *N* — это составное SLA для приложения, развернутого в одном регионе, а *R* — это количество регионов, в которых развернуто приложение. Ожидаемая вероятность того, что приложение не будет работать во всех регионах одновременно, составляет ((1 &minus; N) ^ R).
...
При выборе парных регионов убедитесь, что в обоих доступны необходимые службы Azure. Список доступных служб по регионам см. на странице [Доступность продуктов по регионам](https://azure.microsoft.com/global-infrastructure/services/). Важно также правильно выбрать топологию развертывания для аварийного восстановления, в особенности при низких показателях RPO и RTO. Чтобы в регионе отработки отказа хватало емкости для рабочей нагрузки, выберите топологию "активный — пассивный" (полная реплика) или топологию "активный — активный". Имейте в виду, что эти топологии развертывания могут повысить сложность и затраты, так как ресурсы в дополнительном регионе подготавливаются заранее и могут находиться в состоянии простоя. Дополнительные сведения см. в разделе [Топологии развертывания для аварийного восстановления][deployment-topologies].
...
Чтобы избежать этого, вы можете разделить систему на изолированные группы, чтобы сбой в одном разделе не разрушал всю систему. Такой подход иногда называют [шаблоном отсеков][bulkhead-pattern].
...
* Для автоматизации подготовки ресурсов Azure можно использовать [Terraform][terraform], [Ansible][ansible], [Chef][chef], [Puppet][puppet], [PowerShell][powershell], [CLI][cli] или [шаблоны Azure Resource Manager][template-deployment].
* Используйте [настройку требуемого состояния службы автоматизации Azure][dsc] (DSC) для настройки виртуальных машин. Для виртуальных машин Linux можно использовать [Cloud-init][cloud-init].
* Вы можете автоматизировать развертывание приложений с помощью [Azure DevOps Services][azure-devops-services] или [Jenkins][jenkins].
...
Независимо от того, какой подход вы выберете, убедитесь, что можете вернуться к последнему известному корректному развертыванию на случай, если новая версия не работает. Также разработайте стратегию для отката изменений в базе данных и любых других изменений в зависимых службах. Если возникают ошибки, журналы приложений должны показать, какая версия вызвала ошибку.

## <a name="monitor-to-detect-failures"></a>Мониторинг сбоев
Мониторинг крайне важен для обеспечения устойчивости. Если происходит какой-либо сбой, вам нужно об этом знать и понять его причину. 

Мониторинг крупномасштабной распределенной системы представляет собой нетривиальную задачу, Представим приложение, которое работает на нескольких десятках виртуальных машин. Невозможно войти в каждую виртуальную машину по одной за раз, и просмотреть файлы журналов, пытаясь устранить проблему. Кроме того, количество экземпляров виртуальных машин может быть непостоянным. Они добавляются и удаляются по мере того, как приложение масштабируется, и иногда может произойти сбой экземпляра, который необходимо повторно подготовить. Кроме того, типичное облачное приложение может использовать несколько хранилищ данных (хранилище Azure, база данных SQL, Cosmos DB, кэш Redis), а одно действие пользователя может охватывать несколько подсистем. 

Процесс наблюдения можно считать конвейером с несколькими отдельными этапами:

![Составное соглашение об уровне обслуживания](./images/monitoring.png)

* **Инструментирование**. Необработанные данные для мониторинга поступают из различных источников, включая [журналы приложений](/azure/application-insights/app-insights-overview?toc=/azure/azure-monitor/toc.json), [метрики производительности ОС](/azure/azure-monitor/platform/agents-overview), [ресурсы Azure](/azure/monitoring-and-diagnostics/monitoring-supported-metrics?toc=/azure/azure-monitor/toc.json), [подписки Azure](/azure/service-health/service-health-overview) и [клиенты Azure](/azure/active-directory/reports-monitoring/howto-integrate-activity-logs-with-log-analytics). Большинство служб Azure предоставляют [метрики](/azure/azure-monitor/platform/data-collection), которые вы можете настроить для анализа проблем и определения их причин.
* **Сбор и хранение**. Необработанные данные иснтрументирования могут храниться в разных местах и ​​в различных форматах (например, журналы трассировки приложений, журналы IIS, счетчики производительности). Данные из этих разрозненных источников собираются, консолидируются и помещаются в надежные хранилища, такие как Application Insights, метрики Azure Monitor, Работоспособность служб, учетные записи хранения и Log Analytics.
* **Анализ и диагностика**. После консолидирования данных в этих различных хранилищах их можно проанализировать для устранения неполадок и получения общего представления о работоспособности приложения. Поиск данных в Application Insights и Log Analytics можно выполнять с помощью [запросов Kusto](/azure/log-analytics/log-analytics-queries). Помощник по Azure предоставляет рекомендации с акцентом на [устойчивость](/azure/advisor/advisor-high-availability-recommendations) и [оптимизацию](/azure/advisor/advisor-performance-recommendations). 
* **Визуализация и оповещения**. На этом этапе данные телеметрии представлены таким образом, что оператор может быстро заметить проблемы или тенденции. Примерами реализации могут служить панели мониторинга или оповещения по электронной почте. [Панели мониторинга Azure](/azure/azure-portal/azure-portal-dashboards) позволяют создать единое представление графиков мониторинга, полученных из Application Insights, Log Analytics, метрик Azure Monitor и Работоспособности служб. [Оповещения Azure Monitor](/azure/monitoring-and-diagnostics/monitoring-overview-alerts?toc=/azure/azure-monitor/toc.json) позволяют создавать оповещения о работоспособности служб и ресурсов.
...
* **Оповещения.** Отслеживайте приложение на предмет предупреждающих знаков, которые могут потребовать активного вмешательства. Например, если вы видите, что база данных SQL или база данных Cosmos DB постоянно регулирует ваше приложение, вам может потребоваться увеличить емкость базы данных или оптимизировать ваши запросы. В этом примере, несмотря на то что приложение может обрабатывать ошибки регулирования прозрачно, ваша телеметрия должна все равно вызывать оповещение, чтобы вы могли предпринять ответные меры. Рекомендуется настроить оповещения по метрикам ресурсов Azure и журналам диагностики на предмет достижения пороговых значений и квот, заданных для служб. Мы рекомендуем настроить оповещения по метрикам, так как они имеют меньшую задержку, чем журналы диагностики. Кроме того, в Azure можно получить некоторое представление о работоспособности с помощью [функции "Работоспособность ресурсов Azure"](https://docs.microsoft.com/en-us/azure/service-health/resource-health-checks-resource-types), что может помочь в регулировании служб Azure.    
* **Отработка отказа**. Подготовьте стратегию аварийного восстановления для своего приложения. Стратегия будет зависеть от вашего соглашения об уровне обслуживания. Для большинства сценариев подойдет конфигурация "активный — пассивный". Дополнительные сведения см. в разделе [Топологии развертывания для аварийного восстановления](./disaster-recovery-azure-applications.md#deployment-topologies-for-disaster-recovery). Большинство служб Azure поддерживают отработку отказа, выполняемую вручную или автоматически. Например, приложения IaaS используют [Azure Site Recovery](/azure/site-recovery/azure-to-azure-architecture) на уровнях веб-приложения и логики, а [группы доступности SQL AlwaysOn](/azure/virtual-machines/windows/sql/virtual-machines-windows-portal-sql-availability-group-dr) — на уровне базы данных. [Диспетчер трафика](https://docs.microsoft.com/en-us/azure/traffic-manager/traffic-manager-overview) обеспечивает автоматическую отработку отказа во всех регионах.
* **Проверка готовности к работе**. Выполните тест готовности к отработке отказа в дополнительный регион и восстановлению размещения в основном регионе. Многие службы Azure поддерживают отработку отказа вручную или тестовую отработку отказа для аварийного восстановления. Кроме того, можно имитировать сбой, завершив работу служб или удалив их.
* **Проверка согласованности данных**. Если сбой происходит в хранилище данных, может возникнуть несогласованность данных, когда хранилище снова станет доступным, особенно если данные были реплицированы. В случае служб Azure, выполняющих репликацию между регионами, необходимо учитывать показатели RTO и RPO для оценки объема данных, которые будут потеряны в результате сбоя. Изучите соглашения об уровне обслуживания для служб Azure, чтобы выяснить, выполняется ли отработка отказа между регионами вручную или инициируется корпорацией Майкрософт. Решение о выполнении отработки отказа для некоторых служб принимает корпорация Майкрософт. Майкрософт может отдать приоритет восстановлению данных в основном регионе, выполняя отработку отказа в дополнительный регион только, если данные в основном регионе восстановить невозможно. Например, такая модель предусмотрена для [геоизбыточного хранилища](/azure/storage/common/storage-redundancy-grs) и [Key Vault](/azure/key-vault/key-vault-disaster-recovery-guidance).
* **Восстановление из резервной копии**. В некоторых сценариях восстановление из резервной копии возможно только в исходном регионе. Например, так работает [служба Azure Backup](/azure/backup/backup-azure-vms-first-look-arm). Другие службы Azure обеспечивают георепликацию резервных копий, как например [кэш Redis для Azure](/azure/redis-cache/cache-how-to-geo-replication). Резервное копирование предназначено для защиты от случайного удаления или повреждения данных и для восстановления приложения до работоспособного состояния, зафиксированного в прошлом. Таким образом, хотя в некоторых случаях резервное копирование можно использовать как решение для аварийного восстановления, обратное не всегда верно: аварийное восстановление не обеспечивает защиту от случайного удаления или повреждения данных.  

Документируйте и протестируйте план аварийного восстановления. Оцените влияние сбоев приложения на бизнес. Максимально автоматизируйте процесс и документируйте любые шаги, выполняемые вручную, такие как переход на другой ресурс вручную или восстановление данных из резервных копий. Регулярно тестируйте процесс аварийного восстановления для проверки и улучшения плана. Настройте оповещения для служб Azure, используемых вашим приложением.
